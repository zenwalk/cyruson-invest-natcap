...
# a single annual raster. Based on WorldClim data and file naming.
#
# Extraterrestrial radiation is a single value per month
#
...
##        eto_annual = postprocws + "eto" + Suffix + ".tif"
    ...
    try:
        ...
        # tmin_1.tif...tmin_12.tif, tmax_1.tif...tmax_12.tif prec_1.tif...prec_12.tif
        
        tmin_fname_pre = "tmin"
        tmax_fname_pre = "tmax"
        precip_fname_pre = "prec"
        # IF FORMALIZING THIS, HAVE THIS BE A USER INPUT
        tile_no = "18"
        ...
        for month in range (1, 13):
            try:
                ...
                tmin_x10 = input_dir + os.sep + tmin_fname_pre + str(month) + "_" + tile_no + ".tif"
                if not gp.exists(tmin_x10):
                    gp.AddError("\nError: Tmin file " + tmin_x10 + " does not exist")
                    raise Exception
                tmin = interws + "tmin_" + str(month)
##                gp.Divide_sa(tmin_x10, "10.0", tmin)
                gp.SingleOutputMapAlgebra_sa("FLOAT(" + tmin_x10 + ") / 10.0", tmin)
                
                tmax_x10 = input_dir + os.sep + tmax_fname_pre + str(month) + "_" + tile_no + ".tif"
                if not gp.exists(tmax_x10):
                    gp.AddError("\nError: Tmax file " + tmax_x10 + " does not exist")
                    raise Exception
                tmax = interws + "tmax_" + str(month)
##                gp.Divide_sa(tmax_x10, "10.0", tmax)
                gp.SingleOutputMapAlgebra_sa("FLOAT(" + tmax_x10 + ") / 10.0", tmax)
                
                precip = input_dir + os.sep + precip_fname_pre + str(month) + "_" + tile_no + ".tif"
                ...
                gp.SingleOutputMapAlgebra_sa("(" + tmin + " + " + tmax + ") / 2.0", tavg)
